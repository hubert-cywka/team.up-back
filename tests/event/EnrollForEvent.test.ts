...
describe('Testing enroll for event use case', () => {
  afterEach(async () => {
  ...
  describe(`POST ${UseCase.PATH_EVENT_ENROLLMENT} as any authenticated user`, () => {
    ...
    it(`should return ${HTTPStatus.CONFLICT} if user is already enrolled`, async () => {
      const sportEvent = await UseCase.findFirstInCollection(UseCase.EVENTS_COLLECTION);
      const requestPath = UseCase.PATH_EVENT_ENROLLMENT.replace(':id', sportEvent.disciplineId.toString()).replace(
        ':eventId',
        sportEvent._id.toString()
      ...
      await UseCase.user.post(requestPath).expect(HTTPStatus.OK);
      await UseCase.user.post(requestPath).expect(HTTPStatus.CONFLICT);
    });

    it(`should not enroll user for event if user is already enrolled`, async () => {
      const sportEvent = await UseCase.findFirstInCollection(UseCase.EVENTS_COLLECTION);
      const requestPath = UseCase.PATH_EVENT_ENROLLMENT.replace(':id', sportEvent.disciplineId.toString()).replace(
        ':eventId',
        sportEvent._id.toString()
  ...
  describe(`POST ${UseCase.PATH_EVENT_ENROLLMENT} as unauthenticated user`, () => {
    it(`should always return ${HTTPStatus.UNAUTHORIZED} `, async () => {
      ...
      await UseCase.unauthenticated.post(requestPath).expect(HTTPStatus.UNAUTHORIZED);
    });
  });
});
